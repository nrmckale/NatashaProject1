...
case $choice in
  1)
    ...
    then 
      ...
      if [ ! -d /splunk/etc ] || [ ! -d /splunk/var ]
      then
        printf '\n%s\n\n' "Creating /tmp directories..."
        if mkdir -p /splunk/etc /splunk/var &> /dev/null
        then
          printf '\n%s\n\n' "Created Splunk Log Directories."
        else
          printf '\n%s\n\n' \
          "Cannot create splunk directories" \
          "Run 'sudo mkdir -p /splunk/etc /splunk/var' to see error."
          exit
        fi
      else
      ...
      if docker run --name splunk \
        ...
        --mount type=bind,source=/splunk/etc,target=/opt/splunk/etc \
        --mount type=bind,source=/splunk/var,target=/opt/splunk/var \
  ...
  3)
    ...
    then
      ...
      rm -rf /splunk/etc &> /dev/null
      rm -rf /splunk/var &> /dev/null
    else
